  ...
  function toggleFavorite(movie) {
    setFavorites(prev => {
      ...
      const next = exists
        ...
        : [...prev, { id: movie.id, title: movie.title, poster_path: movie.poster_path, release_date: movie.release_date, vote_average: movie.vote_average }];
      logEvent(exists ? "favorite_remove" : "favorite_add", { movieId: movie.id, title: movie.title });
      return next;
    });
  }

  // Restore last search on first visit if we have a query but no results yet
  useEffect(() => {
    ...
    <section className="page">
            ...
            <li className="card" key={m.id} style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: 12 }}>
              ...
              <div style={{ display: "grid", gap: 6 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                  <strong style={{ fontSize: "1rem" }}>{m.title}</strong>
                  <button className="icon-btn" onClick={() => toggleFavorite(m)} title={favoriteSet.has(m.id) ? "Remove favorite" : "Add favorite"}>
      ...
      {totalPages > 1 && (
        <div className="toolbar" style={{ justifyContent: "flex-end" }}>
          <button className="btn" onClick={() => { const p = Math.max(1, page - 1); runSearch(p); }} disabled={page <= 1}>Prev</button>
          <button className="btn" onClick={() => { const p = Math.min(totalPages, page + 1); runSearch(p); }} disabled={page >= totalPages}>Next</button>
        </div>
      )}
